...
public class LoadDatabase {
    ...
    CommandLineRunner initDatabase(IUserRepository userRepository, IEnergySourceRepository energySourceRepository) {
        return args -> {
            ...
            EnergySource solarEnergy = new EnergySource("Solar", 1000.0, true, 1000.0, user);
            EnergySource windEnergy = new EnergySource("Wind", 1200.0, true, 950.0, user);
            EnergySource hydroEnergy = new EnergySource("Hydro", 899.0, true, 899.0, admin);
            EnergySource geothermalEnergy = new EnergySource("Geothermal", 1350.0, true, 1000.0, admin);
